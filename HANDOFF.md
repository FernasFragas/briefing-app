...
`provider-alternatives-wiring-tasks.md` and `tasks/`.

**Baseline after G1–G5: 611 tests passing, zero failures** (verified 2026-09-06, full
suite run). Earlier figures in this file — 517, 520, 535, 540, 569 — are historical.
Verify before changing anything:

```bash
PYTHONPATH=src .venv/bin/python -m pytest        # expect 611 passed
```
...
disproved 2026-09-06.** `docs/alternatives/directional-probability.md`.
...
- **G1 — diagnosed, and there was no defect.** The opening claim, that grades could not be
  ...
  `0.60 / 0.40` otherwise. It is written down in `docs/SPEC-graded-ideas-report.md` and was
...
`docs/alternatives/still-failing.md` still describe the 4-sector config and the
...
**`docs/alternatives/q6-alpha-vantage-allocation.md`**.
...
- **N4b** — AV news now requests `limit=1000`. Same-ticker probe, NVDA on 2026-09-03:
  ...
  refresh policy is Phase C in `provider-alternatives-wiring-tasks.md`.
...
Evidence: `docs/alternatives/pa6-political-and-retail.md`.

### P2 — Finnhub news + analyst (I6, I9, I11 / PB3, PB9-analyst) — ✅ Done 2026-09-01

Full evidence: `docs/alternatives/pb3-finnhub-news-and-analyst.md`. In short:
...
Evidence: `docs/alternatives/p4-macro-calendar.md`. `macro_calendar` is built from FRED
...
- **`S_F` / 13F** — blocked by design, not wiring. `docs/alternatives/pb1-13f-blocker.md`.
- **FCA / EU sources** — `pb5-fca-deferred.md`, `pa5-eu-sources.md`.
- **CBOE failover (Tradier)** — redundancy for a source that has not failed.
- **Phase C** — `PC1` is regenerated in `docs/alternatives/still-failing.md` from the
...
> **These are answered.** `docs/POST-P3-DECISIONS.md` works through Q1–Q6 against live
...
**Q5 — Grade bands. Answered in `docs/POST-P3-DECISIONS.md`: leave the edges alone.**
...
**Plan: `docs/alternatives/q6-alpha-vantage-allocation.md`** (see N4 above). Two corrections
...
1. **Two unrelated wave numberings.** `tasks/` uses **Stages 1–5** (`T1`–`T9`, the graded
   report — complete). `provider-alternatives-implementation.md` uses **Waves 0–3**
...
| File | What it is |
|---|---|
| `provider-alternatives-wiring-tasks.md` | The audit, coverage matrix and research verdicts — the **why**. Start here |
| `provider-alternatives-implementation.md` | Tickets `I0`–`I16` with status — the **how** |
| `docs/alternatives/*.md` | 17 files: PA1–PA10 verdicts, two deferral notes, the PB3 and P4 outcome notes, and `still-failing.md` (PC1) |
| `docs/SOURCE_STATUS.md` | Endpoint statuses. Header and active summary table are regenerated from the 2026-09-03T13:10:58 credentialed deep preflight over 50 endpoints; historical banners below explain the 08-31 / 09-02 changes |
| `docs/POST-P3-DECISIONS.md` | **Answers Q1–Q6** from live output, with a recommended order. Read it before acting on §3 |
| `docs/alternatives/still-failing.md` | `PC1`: regenerated from deep preflight `2026-09-03T13:10:58` plus live run `daily-2026-09-03-c7e82663` |
| `docs/alternatives/q6-alpha-vantage-allocation.md` | **N4 plan**: which legs AV should carry, given 25 requests/day against 24 tickers |
| `docs/SPEC-graded-ideas-report.md` | Spec for the graded report — built and shipped, and **current**: it carries the G1–G5 amendments (band-dependent weights, `DIRECTIONAL_FULL_CONVICTION`, the published `direction` field) |
| `tasks/plan.md`, `tasks/todo.md` | The graded report's plan and task list — complete. `todo.md` carries a post-09-06 amendment recording what G1–G5 changed under T4 and T5 |
| `implementation-tasks.md` | The original T0–T14 build plan. **Partly stale**: T13/T14 overlap the newer provider docs, and it still asserts the AV quota resets at UTC midnight, which was withdrawn |
