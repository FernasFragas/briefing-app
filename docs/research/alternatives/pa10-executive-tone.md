...
`docs/research/SOURCE_STATUS.md`), so falling back is correct and no amount of parameter fiddling
...
2. **If it is gated, close through PC3**: record `executive_tone` permanently `n/a` in
   `docs/research/SOURCE_STATUS.md`, make it degrade with a named reason rather than renormalizing
