...
Source plan: `docs/archive/options-briefing-system-plan.md` — see also `docs/archive/implementation-tasks.md`, `docs/research/SOURCE_STATUS.md`, `config/source_registry.yaml`.

**Implementation tickets live in `docs/archive/provider-alternatives-implementation.md`.** This file is the audit, the coverage matrix and the research verdicts — the *why*. That one is the *how*.
...
**Phase A is complete.** All ten tasks have an evaluation in `docs/research/alternatives/` with a
...
| Fixture IV surface, float-noise crash, divergence threshold | Three defects fixed; see the graded-report work in `tasks/archive/phase-3/todo.md` |
...
| Institutional — S_F | `[alpha_vantage, fmp]` | ❌ no working source | **Blocked by design** — needs a filer universe and a two-quarter diff. See `docs/research/alternatives/pb1-13f-blocker.md` |
...
evidence and rejected candidates, are in `docs/research/alternatives/`.
...
- Every task PA1–PA10 has an evaluation file in `docs/research/alternatives/` with a verdict.
...
| **PB3** | Adopt one PA3 winner as a second news source: client + normalizer + pipeline merge so news sentiment survives an Alpha Vantage quota outage | ✅ **Done 2026-09-01.** Finnhub leads the chain; tone derived locally because the free tier does not score its own articles. `docs/research/alternatives/pb3-finnhub-news-and-analyst.md` |
...
| **PB5** | Wire FCA shorts; adopt any PA5 source removing the manual-capture dependency | ⏭ **Deferred with reason.** FCA covers UK issuers and the universe has **zero UK names**; there is also no FCA client to "wire". PA5 is a structural reject. See `docs/research/alternatives/pb5-fca-deferred.md` |
...
- `docs/research/SOURCE_STATUS.md` regenerated to reflect the new source set.
...
- `docs/archive/still-failing.md` exists, is dated, and was generated from a preflight report plus a live run.
...
- `pytest` green; preflight `ok` for every newly adopted source; `docs/research/SOURCE_STATUS.md` regenerated.
...
6. ~~**Regenerate `docs/research/SOURCE_STATUS.md`.**~~ ✅ **Done 2026-09-03.** Header and summary
...
**Phase C has run once end to end.** `docs/archive/still-failing.md` was regenerated
