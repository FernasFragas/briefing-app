...
uses Stages 1–5 (`T1`–`T9`) and `docs/archive/provider-alternatives-implementation.md` uses Waves 0–3
...
`docs/research/alternatives/pa5-eu-sources.md` rejects EU coverage, and `HANDOFF.md` Q2 records
...
an entitlement refusal — the same quota-vs-entitlement trap `docs/research/SOURCE_STATUS.md` already
...
| **`S_I` insider** | ❌ Still absent for EU | EDGAR is US-only; EU MAR Art. 19 remains `browser_required` with a loader that has never been fed. Tracked as `PC1-PF-14` in `docs/archive/still-failing.md` |
...
| File | Change |
|---|---|
| `docs/research/alternatives/pa5-eu-sources.md` | Add a dated banner: the options verdict stands, the "no EU data" conclusion is narrowed to "no EU *options* data" |
| `docs/research/SOURCE_STATUS.md` | New banner in the existing style — AV serves EU venue symbols, Finnhub serves EU issuers via US ADRs |
...
| `docs/archive/provider-alternatives-wiring-tasks.md` | The EU row of the coverage matrix moves from "capture-gated" to "capture-gated for `S_O` only". Note that **PB4's EU remainder and A2 are still open** there, and Phase C has since run: `PC1-PF-12`…`PC1-PF-15` are the EU capture rows |
| `docs/archive/still-failing.md` | *(added 2026-09-03)* The Phase C ledger, and the current gate for this work. EU captures are `PC1-PF-12`…`PC1-PF-15`, open. If option 0 in §6 is taken, the four EU names become live rows and belong on the next regeneration |
| `config/candidates.example.yaml` | *(added 2026-09-03)* E5 — `ASML.AS` and `SIE.DE` mappings; D5 |
| `docs/research/alternatives/pa2-iv-history-and-chain-failover.md` | *(added 2026-09-03)* If E1 finds a resolvable US chain for ASML, record there that it was **rejected as the wrong instrument** (D4), so the next audit does not re-find it and read it as a win |
