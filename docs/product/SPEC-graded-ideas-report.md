...
- No new providers or credentials. This is presentation and grading over data the pipeline
  already computes.
...
now uses one of them for conviction (`S_CTE` alignment), prints one as context
(`P(thesis band)`), and uses the third (`tier`) as a displayed-letter cap, so an idea
built on weaker data can retain numeric ranking resolution without presenting as
high-certainty.

### Inputs (all already computed — nothing new is fetched)

| Input | Source | Role | Range |
|---|---|---|---|
| `P` — probability of the thesis band | `Setup.scenario_table` (`strategy/scenarios.py`) | Display context only | 0..1 |
| `S_CTE` — multi-factor signal | `Setup.s_cte` / `ScoringResult` | Score input | −1..+1 |
| `tier` — data confidence | `ConfidenceTier` (`models/scoring.py:50`) | Displayed-letter cap | A/B/C |
| `confidence_multiplier` — gate penalty | `CandidateGateResult` (`models/gate.py:112`) | Score penalty | 0..1 |
...
No scenario table → `P = None`, but the grade is still computed when `S_CTE` exists. The
grade result records `NO_SCENARIO_TABLE`, and the dashboard row publishes
`thesis_probability = null`, so the missing probability is visible beside the grade.
Missing `S_CTE` still means `grade_score = None` with reason `NO_S_CTE`, because ALIGN has
no score input left to use.

### Amendment, accepted 2026-09-07: probability is display context only

As first written, this spec refused to infer a grade from `S_CTE` alone because the report
named `P(thesis band)` as an input. The 2026-09-06 live run disproved that principle:
directional probabilities were nearly flat around 0.50, while neutral probabilities paid
rows for the definition of the neutral band even when `S_CTE` contradicted the thesis. The
grade is therefore ALIGN: `100 * alignment`, less penalties. `P(thesis band)` remains in
the table so the reader can see the scenario context, but it contributes zero points.
...
every other setup type; direction is the fallback, not a replacement. Tier letter caps
still apply, so this cannot promote a watchlist row into high-certainty presentation.
...
score      = max(raw - penalties, 0)
letter     = letter_for_score(score), capped by tier
```

The old `probability_weight`, `alignment_weight` and `directional_probability_weight`
fields remain in `report.grading:` for configuration compatibility, but the grade no
longer reads them. `GradeResult.probability_weight` reports `0.0` and
`GradeResult.alignment_weight` reports `1.0`, reflecting what was actually applied. The
penalty magnitudes still come from `report.grading:`.
...
instead. Dividing it by `NEUTRAL_BAND` saturated alignment at `|S_CTE| >= 0.15`, so the
score reached full conviction too early. 0.35 is the smallest denominator above the
largest `|S_CTE|` observed on a live run (0.317), so no real row clips and the weak tail is
depressed as little as possible; it is a named constant so it can be re-tuned once more
runs exist.

The denominator is no longer trying to compensate for a probability term. It exists only
to keep the directional alignment branch ordered across the observed live `S_CTE` range.
...
### Bands and tier letter caps

The score bands stay on the full 0-100 range. Re-cutting them by the old Tier B ceiling
was rejected because it would relabel Tier B rows as `A+` and Tier C rows as `B`, which is
the exact presentation problem the tier mechanism exists to prevent.
...
The tier is now a cap on the displayed letter, not on `grade_score`:

| Tier | Displayed-letter cap | Score effect |
|---|---|---|
| A | A+ | None |
| B | B+ | None; a score above 81 still publishes at full value |
| C | C | None; a score above 57 still publishes at full value |

This keeps ranking resolution for high-conviction Tier B ideas, e.g. 93.1 / 84.5 / 83.6
instead of a three-way tie at 81.0, while still preventing any Tier B idea from rendering
as `A` or `A+` and any Tier C idea from rendering above `C`.

Known consequence: `grade_score` and `grade_letter` can disagree across tiers. A Tier C
row scoring 79.0 displays `C`, while a Tier B row scoring 74.0 displays `B+`. Ordering
therefore uses numeric `grade_score`; the capped letter is a presentation of confidence
tier, not a secondary sort key.
...
`docs/archive/provider-alternatives-implementation.md` — it only stops the report from concealing it.
...
| Level | Coverage |
|---|---|
| Unit — `test_grading.py` | Every band boundary (34/35, 49/50, 57/58, 65/66, 73/74, 81/82, 89/90); Tier B and Tier C letter caps; P-selection for all 10 `SetupType` values; alignment sign logic; both penalties; `P is None` still scored when `S_CTE` exists; `S_CTE is None` → `n/a` |
...
**Required:** a test asserting a Tier C idea with `P = 0.95` displays no higher than `C`.
...
- **Ask first:** changing penalty magnitudes from the defaults above;
...
7. `dashboard.json` validates as `dashboard.v2`, retains all v1 fields, and every grade in
   it recomputes from the row's `direction`, `s_cte`, `tier`, and `grade_penalty_total`.
...
wiring in `docs/archive/provider-alternatives-implementation.md` lands — **I2** (Wave 0, SEC EDGAR →
...
1. **Persist grades?** Adding `grade_letter`/`grade_score` to `setup_signal` (migration
...
2. **Divergence penalty of 10** — when implied and measured sigma disagree, is a 10-point
